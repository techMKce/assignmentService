...
public class SubmissionController {
                ...
                Assignment assignment = assignmentService.getAssignmentById(
                                Optional.ofNullable(request.getAssignmentId())
                                                ...
                                                .orElseThrow(() -> new IllegalArgumentException(
                                                                "Assignment ID cannot be null or blank")))
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Assignment not found for ID: " + request.getAssignmentId()));
                Submission submission = submissionService.updateSubmissionStatus(
                                Optional.ofNullable(request.getSubmissionId())
                                                .filter(id -> !id.isBlank())
                                                .orElseThrow(() -> new IllegalArgumentException(
                                                                "Submission ID cannot be null or blank")),
        ...
        @GetMapping("/download")
        public ResponseEntity<?> downloadSubmission(@RequestParam("submissionId") String submissionId)
                        throws IOException {
                logger.info("Attempting to download file for submissionId: {}", submissionId);

                if (submissionId == null || submissionId.isBlank()) {
                        logger.error("Submission ID is null or blank");
                        return ResponseEntity.badRequest()
                                        .body(new ErrorResponse("Submission ID cannot be null or blank"));
                }

                GridFSFile gridFSFile = fileService.getFileBySubmissionId(submissionId);
                if (gridFSFile == null) {
                        logger.error("File not found for submissionId: {}", submissionId);
                ...
                return ResponseEntity.ok()
                                .contentType(MediaType
                                                .parseMediaType(gridFSFile.getMetadata().getString("_contentType")))
                                .header(HttpHeaders.CONTENT_DISPOSITION,
                                                "attachment; filename=\"" + gridFSFile.getFilename() + "\"")
                                .body(new InputStreamResource(
                                                fileService.getGridFsTemplate().getResource(gridFSFile)
                                                                .getInputStream()));
        }
        ...
        public ResponseEntity<?> deleteSubmission(@RequestBody DeleteSubmissionRequest request) {
                ...
                String studentRollNumber = Optional.ofNullable(request.getStudentRollNumber())
                                .filter(id -> !id.isBlank())
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "Student Roll Number cannot be null or blank"));
        ...
        public ResponseEntity<?> getStudentProgress(@RequestParam("studentRollNumber") String studentRollNumber,
                ...
                double progressPercentage = assignments.isEmpty() ? 0.0
                                : submissionService.countByStudentRollNumberAndAssignmentIds(
                                                validRollNumber,
                                                assignments.stream().map(Assignment::getAssignmentId).toList()) * 100.0
                                                / totalAssignments;
                ...
                return ResponseEntity.ok(Map.of(
                                "message",
                                assignments.isEmpty() ? "No assignments found for the course"
        ...
        public ResponseEntity<?> getAllStudentProgress(@PathVariable String courseId) {
                ...
                List<SubmissionService.StudentProgress> progressList = submissionService
                                .getStudentProgressForCourse(validCourseId);
        ...
        public ResponseEntity<?> downloadStudentProgress(@PathVariable String courseId) {
                return Optional.ofNullable(courseId)
                                ...
                                .map(id -> {
                                        try {
                                                // Fetch Course Name and Course Faculty
                                                List<Assignment> assignments = assignmentService
                                                                .getAssignmentsByCourseId(id);
                                                String courseName = assignments.isEmpty() ? "Unknown"
                                                                : assignments.get(0).getCourseName();
                                                String courseFaculty = assignments.isEmpty() ? "Unknown"
                                                                : assignments.get(0).getCourseFaculty();

                                                // Sanitize for filename
                                                String sanitizedCourseName = courseName.replaceAll("[^a-zA-Z0-9]", "_");
                                                String sanitizedCourseFaculty = courseFaculty.replaceAll("[^a-zA-Z0-9]",
                                                                "_");
                                                String filename = String.format("%s-%s.csv", sanitizedCourseName,
                                                                sanitizedCourseFaculty);

                                                String csvContent = submissionService
                                                                .generateStudentProgressCsvForCourse(id);
        ...
        public ResponseEntity<?> getStudentAverageGrade(@RequestParam("studentRollNumber") String studentRollNumber,
                ...
                double totalGrade = assignmentIds.stream()
                                .map(id -> gradingRepository.findByStudentRollNumberAndAssignmentId(validRollNumber,
                                                id))
